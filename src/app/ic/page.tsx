    ...
    <div className="min-h-screen bg-ops-bg text-ops-text">
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        {cases.length === 0 ? (
          ...
          <>
            {/* Mobile: stacked cards. Hidden on sm+ screens. */}
            <div className="flex flex-col gap-3 sm:hidden">
              {cases.map((c) => (
                <Link
                  key={c.id}
                  href={`/ic/cases/${c.id}`}
                  className="block p-4 rounded-xl border border-ops-border bg-ops-surface"
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-ops-accent font-medium">{c.trackingCode}</span>
                    <span
                  ...
                  <p className="text-sm text-ops-text m-0 mb-1">{c.category}</p>
                  <div className="flex items-center justify-between text-xs text-ops-text-muted">
                    <span>{c.status}</span>
                    <span>{new Date(c.createdAt).toLocaleDateString()}</span>
                  </div>
                  {c.retaliationFlag && (
                    <p className="text-xs text-status-urgent mt-2 mb-0">⚠ Retaliation risk</p>
                  )}
                </Link>
              ))}
            </div>

            {/* Desktop: full table. Hidden below sm breakpoint. */}
            <div className="hidden sm:block rounded-xl border border-ops-border overflow-hidden">
